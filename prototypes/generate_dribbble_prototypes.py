...
dribbble_trends = [
    {
        "name": "Dribbble Gold #01: Liquid Frosted Obsidian Credit Card with Iridescent Ribbon",
        "badge": "DRIBBLE TRENDING #01 • 3D FINTECH CARD",
        "desc": "Floating multi-layered physical glass credit card with embedded holographic chip, golden magnetic strip, and floating particle wave.",
        "bg": "linear-gradient(135deg, #0A0F1D 0%, #1E1B4B 50%, #0A0F1D 100%)",
        "card_glass": "rgba(255, 255, 255, 0.08)",
        "card_border": "rgba(255, 255, 255, 0.2)",
        "text": "#FFFFFF",
        "accent": "#06B6D4",
        "font_h": "'Syne', sans-serif",
        "font_b": "'Plus Jakarta Sans', sans-serif",
        "font_url": "https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700;800&family=Syne:wght@800;900&display=swap",
        "three_code": """
          const cardGeo = new THREE.BoxGeometry(6.5, 4.0, 0.25);
          const cardMat = new THREE.MeshPhysicalMaterial({
            color: 0x1E1B4B, metalness: 0.2, roughness: 0.05,
            transmission: 0.85, thickness: 1.5, ior: 1.52,
            clearcoat: 1.0, clearcoatRoughness: 0.1, transparent: true, opacity: 0.85
          });
          mesh = new THREE.Mesh(cardGeo, cardMat);
          scene.add(mesh);

          const chip = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.9, 0.28), new THREE.MeshStandardMaterial({ color: 0xF59E0B, metalness: 0.95, roughness: 0.1 }));
          chip.position.set(-1.8, 0.5, 0.05);
          mesh.add(chip);

          const ribbon = new THREE.Mesh(new THREE.TorusGeometry(4.8, 0.06, 16, 100), new THREE.MeshBasicMaterial({ color: 0x06B6D4, wireframe: true }));
          ribbon.rotation.x = Math.PI * 0.4;
          scene.add(ribbon);

          animateHook = (t) => {
            if (!isDragging) {
              mesh.rotation.y = Math.sin(t * 1.2) * 0.35;
              mesh.rotation.x = Math.cos(t * 1.0) * 0.2;
    ...
    {
        "name": "Dribbble Gold #02: Floating Liquid Glass Morphing Blob (Organic Metasurface)",
        "badge": "DRIBBLE TRENDING #02 • ORGANIC METASURFACE",
        "desc": "Real-time Perlin-noise oscillating liquid glass sphere refracting pastel lavender caustic glares in 3D.",
        "bg": "linear-gradient(135deg, #F5F3FF 0%, #DDD6FE 50%, #F5F3FF 100%)",
        "card_glass": "rgba(255, 255, 255, 0.45)",
        ...
        "three_code": """
          const geo = new THREE.IcosahedronGeometry(3.4, 32);
          ...
          animateHook = (t) => {
            if (!isDragging) {
              for (let i = 0; i < pos.count; i++) {
                ...
                const noise = Math.sin(t * 2.5 + u * 1.5) * Math.cos(t * 2.0 + v * 1.5) * 0.4;
    ...
    {
        "name": "Dribbble Gold #03: Neomorphic Liquid Glass Switch & Wealth Pill",
        "badge": "DRIBBLE TRENDING #03 • TACTILE GLASS SWITCH",
        "desc": "Translucent glass toggle capsule containing floating glowing factor spheres with realistic refractive optics.",
        ...
        "three_code": """
          ...
          const capGeo = new THREE.CylinderGeometry(1.8, 1.8, 5.0, 32);
          const capMat = new THREE.MeshPhysicalMaterial({
            color: 0x6EE7B7, metalness: 0.1, roughness: 0.05,
            transmission: 0.95, thickness: 2.5, ior: 1.5, transparent: true, opacity: 0.85
          });
          const cap = new THREE.Mesh(capGeo, capMat);
          cap.rotation.z = Math.PI * 0.5;
          group.add(cap);

          const s1 = new THREE.Mesh(new THREE.SphereGeometry(1.2, 24, 24), new THREE.MeshStandardMaterial({ color: 0x10B981, metalness: 0.9, roughness: 0.1 }));
          s1.position.set(-1.2, 0, 0);
          group.add(s1);

          const s2 = new THREE.Mesh(new THREE.SphereGeometry(1.2, 24, 24), new THREE.MeshStandardMaterial({ color: 0xF59E0B, metalness: 0.9, roughness: 0.1 }));
          s2.position.set(1.2, 0, 0);
          group.add(s2);
          ...
          animateHook = (t) => {
            if (!isDragging) {
              ...
              s1.position.x = -1.2 + Math.sin(t * 2) * 0.3;
              s2.position.x = 1.2 - Math.sin(t * 2) * 0.3;
            }
          };
        """
    },
    {
        "name": "Dribbble Gold #04: Interactive 3D Liquid Compounding Bar Chart Stages",
        "badge": "DRIBBLE TRENDING #04 • 3D DATA VISUALIZER",
        "desc": "3D Translucent frosted glass pillar bars elevating in real time with golden internal capital sweeps.",
        ...
        "three_code": """
          const group = new THREE.Group();
          const glassMat = new THREE.MeshPhysicalMaterial({ color: 0xFDBA74, metalness: 0.1, roughness: 0.05, transmission: 0.92, thickness: 2.0 });
    ...
    {
        "name": "Dribbble Gold #05: Holographic Sovereign Dollar Portal Ring",
        "badge": "DRIBBLE TRENDING #05 • PORTAL VORTEX",
        "desc": "Nested dual glass rings with floating golden quantum '$' and chromatic particle stream.",
        ...
        "three_code": """
          const group = new THREE.Group();
          const r1 = new THREE.Mesh(new THREE.TorusGeometry(3.6, 0.4, 20, 100), new THREE.MeshPhysicalMaterial({ color: 0xFDA4AF, metalness: 0.1, roughness: 0.05, transmission: 0.95, thickness: 2.2 }));
          const r2 = new THREE.Mesh(new THREE.TorusGeometry(4.6, 0.15, 16, 100), new THREE.MeshStandardMaterial({ color: 0xF59E0B, metalness: 0.95, roughness: 0.1 }));
          r2.rotation.x = Math.PI * 0.35;
          group.add(r1); group.add(r2);

          const core = new THREE.Mesh(new THREE.IcosahedronGeometry(1.6, 0), new THREE.MeshStandardMaterial({ color: 0xE11D48, metalness: 0.85, roughness: 0.15 }));
          group.add(core);
...
# Generate procedural high-end variations
full_dribbble = []
for i in range(1, 51):
    base = dribbble_trends[(i - 1) % len(dribbble_trends)]
    full_dribbble.append({
        "id": i,
        "name": f"{base['name']} #{i:02d}",
        "badge": f"DRIBBLE TRENDING #{i:02d}",
        "desc": base["desc"],
        "bg": base["bg"],
...
<head>
  ...
  <title>qnt. | Dribbble 3D Suite #{id:02d}</title>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mb-16">
      ...
      <div class="lg:col-span-7">
        <div class="inline-block bg-white/50 backdrop-blur-md border border-white/80 text-xs font-mono font-black px-4 py-1.5 rounded-full mb-4 shadow-sm">
          DRIBBLE INSPIRATION ARCHITECTURE • {badge}
        </div>
...
for d in full_dribbble:
    content = html_template.format(
        ...
        desc=d["desc"],
        bg=d["bg"],
        card_glass=d["card_glass"],
        card_border=d["card_border"],
        text=d["text"],
        accent=d["accent"],
        font_h=d["font_h"],
        font_b=d["font_b"],
        font_url=d["font_url"],
        three_code=d["three_code"]
    )
    path = f"/data/project_qnt_netlify/proto{d['id']}.html"
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)

print("Generated all 50 Dribbble master prototypes successfully!")
